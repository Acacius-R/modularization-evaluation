...
class GA:
    def __init__(self,
                ...
                for index, el in enumerate(gene_space):
                    if type(el) in [list, tuple, range, numpy.ndarray]:
                        if len(el) == 0:
                            ...
                            for val in el:
                                if not (type(val) in [int, float,  numpy.int8, numpy.int16, numpy.int32,
                                                      numpy.int64,  numpy.float16, numpy.float32,
                                                      numpy.float64]):
                                    raise TypeError(
                        ...
                        self.gene_space_nested = True
                    elif not (type(el) in [int, float, numpy.int8, numpy.int16, numpy.int32, numpy.int64,
        ...
        if gene_type in [int, float,  numpy.int8, numpy.int16, numpy.int32, numpy.int64, 
                         numpy.float16, numpy.float32, numpy.float64]:
        ...
        elif type(crossover_probability) in [int, float, numpy.int8, numpy.int16, numpy.int32, numpy.int64,
                                             numpy.float16, numpy.float32, numpy.float64]:
        ...
        if not (self.mutation_type is None):
            if mutation_probability is None:
                ...
                if type(mutation_probability) in [int, float,  numpy.int8, numpy.int16, numpy.int32,
                                                  numpy.int64,  numpy.float16, numpy.float32,
                                                  numpy.float64]:
                ...
                if type(mutation_probability) in [list, tuple, numpy.ndarray]:
                    if len(mutation_probability) == 2:
                        for el in mutation_probability:
                            if type(el) in [int, float,  numpy.int8, numpy.int16, numpy.int32, numpy.int64,
                                             numpy.float16, numpy.float32, numpy.float64]:
                                if el >= 0 and el <= 1:
                                    pass
                                else:
        ...
        if not (self.mutation_type is None):
            if mutation_num_genes is None:
                ...
                if (mutation_type != "adaptive"):
                    ...
                    elif type(mutation_percent_genes) in [int, float,  numpy.int8, numpy.int16, numpy.int32,
                                                          numpy.int64,  numpy.float16, numpy.float32,
                                                          numpy.float64]:
                    ...
                    if type(mutation_percent_genes) in [list, tuple, numpy.ndarray]:
                        if len(mutation_percent_genes) == 2:
                            ...
                            for idx, el in enumerate(mutation_percent_genes):
                                if type(el) in [int, float,  numpy.int8, numpy.int16, numpy.int32,
            ...
            # The mutation_num_genes parameter exists. Checking whether adaptive mutation is used.
                ...
                if type(mutation_num_genes) in [int,  numpy.int8, numpy.int16, numpy.int32, numpy.int64]:
                    if (mutation_num_genes <= 0):
                        self.valid_parameters = False
                        raise ValueError(
                ...
                if type(mutation_num_genes) in [list, tuple, numpy.ndarray]:
                    if len(mutation_num_genes) == 2:
                        for el in mutation_num_genes:
                            if type(el) in [int,  numpy.int8, numpy.int16, numpy.int32, numpy.int64]:
                                if (el <= 0):
                                    self.valid_parameters = False
                                    raise ValueError(
        ...
        # delay_after_gen
        if type(delay_after_gen) in [int, float,  numpy.int8, numpy.int16, numpy.int32, numpy.int64,
                                      numpy.float16, numpy.float32, numpy.float64]:
                ...
                for gene_idx in range(self.num_genes):
                    ...
                    if type(curr_gene_space) in [list, tuple, range]:
                        ...
                        self.population[sol_idx, gene_idx] = self.gene_type(numpy.random.uniform(low=low,
                                                                                                 high=high,
                                                                                                 size=1))
                    elif type(curr_gene_space) in [int, float,  numpy.int8, numpy.int16, numpy.int32,
                                                   numpy.int64,  numpy.float16, numpy.float32,
                                                   numpy.float64]:
    ...
    def roulette_wheel_selection(self, fitness, num_parents):
        ...
        probs_start = numpy.zeros(probs.shape,
                                  dtype=numpy.float64)  # An array holding the start values of the ranges of probabilities.
        probs_end = numpy.zeros(probs.shape,
                                dtype=numpy.float64)  # An array holding the end values of the ranges of probabilities.
        ...
        for offspring_idx in range(offspring.shape[0]):
            ...
            for gene_idx in mutation_indices:
                ...
                if self.gene_space_nested:
                    ...
                    if type(curr_gene_space) in [int, float,  numpy.int8, numpy.int16, numpy.int32,
                                                 numpy.int64,  numpy.float16, numpy.float32, numpy.float64]:

...
def generate_target_module(target_classes, module_mask_path,random_selection = False):
    ...
    if random_selection:
        num_ones = torch.sum(target_module_mask).item()
        random_module_mask = torch.zeros_like(target_module_mask, dtype=torch.int)
        indices = torch.randperm(target_module_mask.numel())[:num_ones]
        random_module_mask[indices] = 1
        target_module_mask = random_module_mask
